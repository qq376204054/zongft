...
;$(function($){
	...
	$('.J_showdialog').live('click', function(){
		...
		$.ajax({
			...
			beforeSend: function () {
				// cog placed
				$.dialog.get(did).content('<p class="loading"><i class="fa fa-spinner fa-spin"></i> 加载中...</p>');
			},
			success: function (data) {
				$.dialog.get(did).content(data);
			},
			error: function (xhr, ajaxOptions, thrownError) {
				$.dialog.get(did).content('<h4 style="margin-top:10px; display:block; text-align:left"><i class="fa fa-warning txt-color-orangeDark"></i> Error 404! 访问的页面不存在.</h4>');
			}
		});
		return false;
	});
    ...
    $.fn.cate_select = function(options) {
        ...
        $.getJSON(uri, {id:pid}, function(result){
            if(result.errNum ===0){
                ...
                $('<option value="'+result.retData[i].id+'">'+result.retData[i].name+'</option>').appendTo(self);
                }
            }
            ...
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.errNum ===0){
                        ...
                        for(var i=0; i<result.retData.length; i++){
                            $('<option value="'+result.retData[i].id+'">'+result.retData[i].name+'</option>').appendTo(_childs);
                        }
...
function loadURL(url, container) {
	...
	var container = arguments[1] ? arguments[1] : '';
