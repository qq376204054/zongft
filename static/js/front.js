    ...
    $.yhxia.ui = {
        ...
        decode_img: function(context) {
            $('.J_decode_img', context).each(function(){
                ...
                $(this).attr('src', $.yhxia.util.base64_decode(uri));  
            });
        }
    },
    $.yhxia.tool = {
            ...
            if(FTXIAER.uid){
                ...
                var update = function() {
                    ...
                    $.getJSON(FTXIAER.root + '/?m=user&a=msgtip', function(result){
                        if(result.errNum === 0){
                            var fans = parseInt(result.retData.fans),
                                atme = parseInt(result.retData.atme),
                                msg = parseInt(result.retData.msg),
                                system = parseInt(result.retData.system),
