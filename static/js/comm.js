...
;(function($){
	//提示
	if('close'==$.yhxia.util.getCookie('leftimg')){
	...
	$('.iknow').live('click', function(){
		$('.tips_goods').css('display','none');
		$.yhxia.util.setCookie('leftimg', 'close');
		$('.tips_goods').remove();
	});
	...
	$('.chaxun').live('click', function(){
		...
		$.getJSON(FTXIAER.chaxunurl, function(result){
			if(result.errNum ===0){
				$.dialog({id:'check_item', title:lang.chaxun, content:result.retData, padding:'', fixed:true, lock:true});
				$.yhxia.item.check_form($('#J_check_item'));
			}else{
    ...
    function() {
        ...
        if (flag == true && $(document).scrollTop() > temp) {
            ...
            if ("fold" == $.yhxia.util.getCookie("fold")) {
    ...
    function() {
        ...
        $('.white_dh #fold').attr('class', 'filter-unfold');
        $.yhxia.util.setCookie("fold", 'fold');
    });
