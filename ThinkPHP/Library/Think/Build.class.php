...
class [MODEL]Model extends Model {
    ...
    static protected $modelBase         =   '<?php
namespace [MODULE]\Model;
use Think\Model;
class [MODEL]Model extends BaseModel {
    ...
    // 创建模型类
    static public function buildBaseModel($module,$model) {
        $file   =   APP_PATH.$module.'/Model/'.$model.'Model'.EXT;
        if(!is_file($file)){
            $content = str_replace(array('[MODULE]','[MODEL]'),array($module,$model),self::$modelBase);
            if(!C('APP_USE_NAMESPACE')){
                $content    =   preg_replace('/namespace\s(.*?);/','',$content,1);
            }
            $dir = dirname($file);
